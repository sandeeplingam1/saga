...
The chat tab uses the approach programmed in `app/backend/approaches/chatreadretrieveread.py`.

1. It calls the OpenAI ChatCompletion API to turn the user question into a good search query, using the prompt and tools from `app/backend/approaches/prompts/chat_query_rewrite.prompty`.
...
The ask tab uses the approach programmed in `app/backend/approaches/retrievethenread.py`.
